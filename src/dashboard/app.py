...
def create_layout():
    ...
    return dbc.Container(
        [
                        ...
                        [
                            html.Div(
                                [
                                    ...
                                    html.P("Analisis de contenido sensible en peliculas", className="lead text-muted"),
                                ],
            ...
            dbc.Row([dbc.Col([create_input_form()], md=12)]),
...
def analyze_movie(n_clicks, movie_id, title, description, genre):
    ...
    if not all([movie_id, title, description, genre]):
        alert = dbc.Alert(
            "Por favor complete todos los campos del formulario", color="warning", dismissable=True, duration=4000
        )
        return {"display": "none"}, {}, None, None, None, alert

    if len(description) < 10:
        alert = dbc.Alert(
            "La descripcion debe tener al menos 10 caracteres", color="warning", dismissable=True, duration=4000
        )
        return {"display": "none"}, {}, None, None, None, alert

    result = api_client.predict(
        movie_id=movie_id.strip(), title=title.strip(), description=description.strip(), genre=genre
    )
    ...
    for pred in predictions:
        ...
        card = create_trigger_card(trigger_key, spanish_name, pred["detected"], pred["probability"], color)
    ...
    success_alert = dbc.Alert("Analisis completado exitosamente", color="success", dismissable=True, duration=3000)

    return ({"display": "block"}, gauge_fig, movie_info, trigger_cards, suitability_text, success_alert)
