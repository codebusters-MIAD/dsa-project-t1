...
def handle_movie_selection(
    n_clicks: List[Optional[int]], ids: List[Dict[str, Any]]
...
def handle_prediction(
    n_clicks: Optional[int], movie_id: Optional[str], cache: Optional[Dict]
) -> Tuple[Any, Dict, Optional[Dict], List]:
    ...
    if movie_id in cache and "prediction" in cache[movie_id]:
        ...
        results = {"movie": cached_data.get("movie_details", {}), "prediction": cached_data.get("prediction", {})}
...
) -> Tuple[Dict[str, str], Optional[Any], Dict, Optional[str], Optional[Any]]:
